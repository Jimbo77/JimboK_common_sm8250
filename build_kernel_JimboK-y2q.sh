...
make O=$(pwd)/out $KERNEL_MAKE_ENV CROSS_COMPILE=$BUILD_CROSS_COMPILE REAL_CC=$KERNEL_LLVM_BIN CLANG_TRIPLE=$CLANG_TRIPLE CFP_CC=$KERNEL_LLVM_BIN vendor/y2q_chn_openx_defconfig
...
mv $(pwd)/out/Image $(pwd)/out/Image-JimboK_y2q
mv $(pwd)/out/dtb.img $(pwd)/out/dtb-JimboK_y2q.img

cp $(pwd)/out/Image-JimboK_y2q ~/build/mkbootimg-master2
cp $(pwd)/out/dtb-JimboK_y2q.img ~/build/mkbootimg-master2

~/build/mkbootimg-master2/make_boot_y2q.sh 1.2.0-y2q
